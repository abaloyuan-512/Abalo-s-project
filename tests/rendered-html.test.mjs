...
test("server-renders the Guanxiang product", async () => {
  ...
  assert.doesNotMatch(html, /inquiry-cloudfall-canvas/);
...
test("question scene keeps cloud and pine motion accessible", async () => {
  ...
  assert.match(appSource, /question-mountain-occluder-v3\.png/);
  assert.match(appSource, /inquiry-cloud-stream-far/);
  assert.match(appSource, /inquiry-cloud-stream-near/);
  assert.match(cssSource, /inquiry-cloud-stream[^}]+repeat-x/);
