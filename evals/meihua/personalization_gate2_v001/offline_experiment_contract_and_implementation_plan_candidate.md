...
- 状态：`APPROVED_STAGE_A_B_IMPLEMENTATION`
- 前置条件：Gate 0与Gate 1均为`PASS`
- 本文用途：定义独立离线实验路径的输入、输出、验证、对照、预算、停止条件与证据包
- 产品负责人授权日期：`2026-07-21`
- 已授权：阶段A离线内容契约实现；阶段B Fake Provider与无网络干跑验证
- 仍不构成：真实模型调用授权、API Key配置授权、费用授权、锁定测试集创建授权或正式产品修改授权

产品负责人原文授权：

> 我批准Gate 2离线实验计划，并授权实施阶段A与阶段B：只开发离线内容契约、实验Validator、Fake Provider和无网络干跑验证；不授权真实模型调用、API费用、锁定测试集创建或正式产品修改。

阶段A/B实现完成后，任何真实模型调用仍需再次获得单独批准；不得从本次授权推导出后续阶段自动获批。
...
## 14. 2026-07-21阶段 C授权补充

产品负责人已在阶段 A/B独立复核通过后，另行明确授权进入阶段 C，并声明OpenAI API账户余额为9美元。

本轮执行采用比第9节更严格的实际硬门：

- 至少7美元继续作为失败复测预留，不得动用；
- 因此本轮可见合成校准最多使用2美元，而不是阶段 C表格中的5美元上限；
- 第一批只使用两组公开可见的合成案例，每组按预先冻结的A/B/C/D顺序运行，其中A不调用模型，B/C/D各一次生成；
- 最大Responses API生成调用数为6，不自动重试、不自动修复；
- 使用当前官方模型标识`gpt-5.6-sol`和标准价格坐标；
- 阶段 C证据仍写在Git仓库外，锁定测试集和阶段 D保持关闭；
- 本授权不允许修改正式网站、V3、确定性排盘、正式Prompt、正式Validator、Release Gate或正式解释知识。

## 15. 2026-07-21阶段 C单次诊断重试补充

首次阶段 C调用在SDK/Pydantic结构验证路径硬停止后，产品负责人另行授权一次诊断重试。本次追加授权采用独立且更窄的硬门：

- 最大Responses API生成调用数为1；
- 新增费用上限为0.35美元，至少7美元预留余额不变；
- 只重试公开可见合成案例`G2CAL-001/B`，不运行其他案例；
- 不自动重试、不自动修复，不覆盖首次失败证据；
- 证据写入新的仓库外目录；
- 无论结果如何，本授权用尽后停止，不进入阶段 D。

实际诊断重试在等待OpenAI API响应时超时。Provider只发起1次请求即停止，没有取得响应ID、Usage或模型正文；因此实际账单金额在本地证据中仍为未知，0.35美元只代表授权硬上限，不代表已确认支出。
