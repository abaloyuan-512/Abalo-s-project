"""Versioned Direct Reading V3 application service with the same-call P9 finale.
...
PROMPT_VERSION = "GUANXIANG_DIRECT_READING_PROMPT_V3_P9_FINALE_SAME_CALL_V1"
OPTIONAL_CONTEXT_PROMPT_VERSION = (
    "GUANXIANG_DIRECT_READING_PROMPT_V3_OPTIONAL_CONTEXT_P9_FINALE_SAME_CALL_V1"
)
P9_FINALE_VERSION = "GUANXIANG_P9_FINALE_V1"
P9_FINALE_HEADING = "## 观象寄语"
MODEL = "gpt-5.6-sol"
...
SYSTEM_PROMPT = (
    ...
    "完成九章正文后，再用观象寄语两句话收束：第一句直接回答原问题，第二句只写最关键的行动边界。"
...
class DirectReadingFinale(StrictModel):
    content_version: Literal[P9_FINALE_VERSION] = P9_FINALE_VERSION
    source: Literal["SAME_PROVIDER_OUTPUT"] = "SAME_PROVIDER_OUTPUT"
...
class DirectReadingResponse(StrictModel):
    ...
    page9_finale: DirectReadingFinale | None = None
...
def build_direct_reading_prompts(
    ...
    section_contract = "\n".join(
        [
            "正文请严格只使用以下九个 Markdown 二级标题，并保持顺序、名称和卦名不变：",
            "## 判断",
            ...
            "## 反向风险",
            "## 哪些现实信号会改变判断",
            "九章正文不得增加、删除或改名标题。动爻章节必须逐字写出程序提供的动爻名与爻辞；不得提及其他爻名。",
            ...
            "九章正文结束后必须追加且只追加一个二级标题 `## 观象寄语`，标题下恰好两行非空正文。",
            "第一行直接回答用户原问题；第二行只写最关键的行动边界。每行4至40个汉字，总计不超过72个字符。",
            "两行不得使用列表、标签、经典原句、日期、必然保证、Markdown 标记或泛化套话，不得复述九章结构。",
        ]
    )
...
def split_page9_finale(output_text: str) -> tuple[str, tuple[str, str] | None]:
    """Separate the frozen two-line finale without changing the nine-section reading."""
    normalized = unicodedata.normalize("NFC", output_text).strip()
    if normalized.count(P9_FINALE_HEADING) != 1:
        return normalized, None
    reading, separator, finale_body = normalized.rpartition(P9_FINALE_HEADING)
    if not separator or not reading.strip():
        return normalized, None
    lines = tuple(line.strip() for line in finale_body.splitlines() if line.strip())
    if len(lines) != 2:
        return reading.strip(), None
    return reading.strip(), (lines[0], lines[1])


def validate_page9_finale(answer: tuple[str, str] | None) -> tuple[str, ...]:
    """Validate only the frozen P9 delivery contract, never synthesize a fallback."""
    if answer is None:
        return ("P9_FINALE_MISSING_OR_MALFORMED",)
    errors: list[str] = []
    if any(not 4 <= len(line) <= 40 for line in answer) or sum(map(len, answer)) > 72:
        errors.append("P9_FINALE_LENGTH")
    joined = "\n".join(answer)
    if any(line.startswith(("-", "*", ">", "#", "1.", "一、", "第一句", "第二句")) for line in answer):
        errors.append("P9_FINALE_FORMAT")
    if _DANGEROUS_MARKUP.search(joined) or _ANY_HTML_TAG.search(joined):
        errors.append("DANGEROUS_MARKUP")
    if _SECRET_PATTERN.search(joined) or SYSTEM_PROMPT[:32] in joined:
        errors.append("SECRET_OR_PROMPT_LEAK")
    if _has_unsupported_date(joined, ""):
        errors.append("UNSUPPORTED_DATE")
    if _has_unqualified_match(_ABSOLUTE_PATTERN, joined):
        errors.append("INEVITABLE_RESULT")
    if _has_unqualified_mind_reading(joined):
        errors.append("THIRD_PARTY_MIND_READING")
    if "《" in joined or "》" in joined or _UNAUTHORIZED_CLASSIC.search(joined):
...
def public_direct_reading_payload(response: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "page9_finale": response.get("page9_finale"),
...
def process_prepared_direct_reading_v2_request(
    ...
    *,
    ...
    reading_text, finale_answer = split_page9_finale(output_text)
    validation_report = validate_direct_reading_release(
        reading_text if finale_answer is not None else output_text,
        question_text=request.question_text,
        facts=facts,
        optional_context=request.optional_context,
    )
    finale_errors = validate_page9_finale(finale_answer)
    validation_ms = int((perf_counter() - validation_started) * 1_000)
    validation_errors = tuple(
        dict.fromkeys((*validation_report.blocking_errors, *finale_errors))
    )
    ...
    assert finale_answer is not None
    return DirectReadingResponse(
        status="SUCCESS",
        direct_reading=DirectReadingContent(text=reading_text, chart_facts=facts),
        page9_finale=DirectReadingFinale(answer=finale_answer),
        audit=_empty_audit(
            request_id=safe_request_id,
            generated_at=generated_at,
            request_sha256=request_sha256,
