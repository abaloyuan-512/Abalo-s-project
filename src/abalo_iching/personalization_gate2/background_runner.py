    ...
    def run(
        ...
        *,
        ...
        if request.metadata.dataset_role is not DatasetRole.CALIBRATION:
            raise Gate2ExecutionBlocked("阶段 C.1只允许可见合成校准集")
        if request.metadata.arm is ExperimentArm.A:
            raise Gate2ExecutionBlocked("阶段 C.1后台运行器不调用A组基线")
        if type(provider) is not OpenAIGate2BackgroundProvider:
            raise Gate2ExecutionBlocked("阶段 C.1只接受隔离的后台Provider")
        if request.metadata.model != provider.model:
            raise Gate2ExecutionBlocked("请求模型与阶段 C.1后台Provider不一致")
        if request.metadata.reasoning_effort != provider.reasoning_effort:
            raise Gate2ExecutionBlocked("请求推理档位与阶段 C.1后台Provider不一致")
        if request.metadata.max_output_tokens != provider.max_output_tokens:
            raise Gate2ExecutionBlocked("请求输出上限与阶段 C.1后台Provider不一致")
