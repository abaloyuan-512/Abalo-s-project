...
FREEZE_PATH = ROOT / "outputs" / "v009_canary_freeze.json"
LEDGER_PATH = ROOT / "outputs" / "v009_canary_call_ledger.json"
MANIFEST_PATH = ROOT / "evals" / "meihua" / "direct_reading_v2_stability_v009" / "candidate_manifest.json"
RESULT_PATH = ROOT / "outputs" / "v009_canary_real_result.json"
...
def main() -> int:
    ...
    if _file_sha(FREEZE_PATH) != ledger["freeze_sha256"]:
        raise RuntimeError("V009_FREEZE_HASH_MISMATCH")
    if _file_sha(MANIFEST_PATH) != ledger.get("candidate_manifest_sha256"):
        raise RuntimeError("V009_CANDIDATE_MANIFEST_MISMATCH")
    if not (MAX_OUTPUT_TOKENS == freeze["case"]["max_output_tokens"] == 12_000):
        raise RuntimeError("V009_OUTPUT_BUDGET_MISMATCH")
    if ledger.get("activation_condition_met") is not True:
        raise RuntimeError("V009_OFFLINE_ACCEPTANCE_NOT_ACTIVATED")
    if (
        ...
        raise RuntimeError("V009_LEDGER_NOT_PRISTINE")
    if RESULT_PATH.exists():
        raise RuntimeError("V009_RESULT_ALREADY_EXISTS")

    case = freeze["case"]
    request_payload = {"question_text": case["question_text"], "numbers": case["numbers"]}
    if _canonical_sha(request_payload) != case["input_sha256"]:
        raise RuntimeError("V009_INPUT_HASH_MISMATCH")
    if "?" in case["question_text"]:
        raise RuntimeError("V009_INPUT_ENCODING_LOSS")
    ...
    try:
        ...
        if prepared.request.question_text != case["question_text"]:
            raise RuntimeError("V009_PREPARED_QUESTION_MISMATCH")
        if prepared.request.numbers != tuple(case["numbers"]):
            raise RuntimeError("V009_PREPARED_NUMBERS_MISMATCH")
    ...
    evidence = {
        "evidence_type": "V009_OUTPUT_BUDGET_REAL_HIGH_CANARY",
        "stage_id": freeze["stage_id"],
        "freeze_version": freeze["freeze_version"],
        "freeze_sha256": ledger["freeze_sha256"],
        "candidate_manifest_sha256": ledger["candidate_manifest_sha256"],
        "slot": 1,
    ...
    print(json.dumps({"status": evidence["status"], "validation_errors": evidence["validation_errors"], "reading_sha256": evidence["reading_utf8_sha256"], "latency_ms": result["audit"].get("latency_ms")}, ensure_ascii=True))
    return 0
