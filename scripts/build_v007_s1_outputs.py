...
def _render_case(case_id: str, evidence: dict) -> dict:
    ...
    expected = [
        ...
        f"动爻：{moving['name']}",
        f"变卦：{changed['name']}",
        *MODEL_TAIL_HEADINGS[1:],
    ]
    parsed = _parse(reading)
    if list(parsed) != expected:
        raise RuntimeError(f"{case_id}:SECTION_MISMATCH:{list(parsed)!r}")
    ...
    if source_sha != reconstructed_sha:
        raise RuntimeError(f"{case_id}:HASH_MISMATCH")

    page = [
        f"# {case_id} 真实 OpenAI high：第八页与第九页",
        "",
    ...
    for label, heading in zip(labels, p8, strict=True):
    ...
    for heading in MODEL_TAIL_HEADINGS:
    ...
    page_path = OUTPUTS / f"v007_{case_id.lower().replace('-', '_')}_page8_page9.md"
    ...
    provenance_path = OUTPUTS / f"v007_{case_id.lower().replace('-', '_')}_provenance.json"
    provenance_path.write_text(json.dumps(provenance, ensure_ascii=False, indent=2), encoding="utf-8")
    return provenance


def main() -> None:
    ledger = json.loads((OUTPUTS / "v007_s1_call_ledger.json").read_text(encoding="utf-8"))
    rendered = []
    for row in ledger["cases"]:
        if row["status"] != "SUCCESS":
            continue
        evidence = json.loads((ROOT / row["result_path"]).read_text(encoding="utf-8"))
        rendered.append(_render_case(row["case_id"], evidence))
    print(json.dumps({"rendered": [item["case_id"] for item in rendered]}, ensure_ascii=True))
