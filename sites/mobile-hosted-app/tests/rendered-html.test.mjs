...
test("server-renders the Guanxiang product", async () => {
  ...
  assert.match(html, /<link[^>]+rel="manifest"[^>]+href="https:\/\/guanxiang-mobile\.abaloyuan\.chatgpt\.site\/manifest-v2\.webmanifest"/);
  ...
  assert.match(source, /function FullscreenRecoveryControl/);
  assert.match(source, /aria-label="恢复全屏显示"/);
  assert.match(source, /document\.addEventListener\("fullscreenchange", sync\)/);
  assert.match(source, /document\.addEventListener\("visibilitychange", sync\)/);
  assert.match(source, /new URLSearchParams\(window\.location\.search\)\.get\("mobile-preview"\) === "1"/);
  assert.match(styles, /-webkit-text-size-adjust:\s*100%/);
  assert.match(styles, /--app-height:\s*100dvh/);
  assert.match(styles, /\.flow-shell \{ height: var\(--app-height\);/);
  assert.match(styles, /\.fullscreen-recovery,[\s\S]*?\.pwa-install-control\s*\{[\s\S]*?position:\s*fixed;/);
  assert.match(styles, /\.p1-mobile-frozen\s*\{[\s\S]*?width:\s*100%;[\s\S]*?height:\s*100svh;/);
  assert.doesNotMatch(styles, /\.p1-mobile-frozen\s*\{[\s\S]*?width:\s*min\(100vw,\s*calc\(100dvh/);
  assert.match(styles, /main\[data-flow-page="2"\] \.method-explainer\s*\{[\s\S]*?top:\s*clamp\(388px,\s*49svh,\s*455px\)/);
  assert.match(styles, /\.final-question-heading\.casting-heading > p\.casting-range-note\s*\{[^}]*top:\s*190px/);
  assert.match(styles, /\.casting-heading \.peony-number-field\s*\{[\s\S]*?top:\s*238px/);
  assert.match(styles, /\.casting-recovery-actions/);
...
test("final question offers a concise user-controlled question decision", async () => {
  ...
  assert.match(cssSource, /final-question-backdrop[^}]+position: fixed[^}]+inset: 0[^}]+width: 100vw[^}]+height: var\(--app-height\)/);
...
test("seventh page opens the page-eight data-model review", async () => {
  ...
  assert.match(appSource, /page8Task\.phase === "FAILED" \|\| page8Task\.phase === "RECOVERABLE"/);
  assert.match(appSource, /onClick=\{retryDirectHighRequest\}>重新尝试/);
  assert.match(appSource, /onClick=\{resumeDirectHighRequest\}>继续查询原任务/);
  assert.match(appSource, /onClick=\{editQuestion\}>返回正问/);
...
test("publishes an installable fullscreen mobile manifest without caching private readings", async () => {
  const manifest = JSON.parse(await fs.readFile(new URL("../public/manifest-v2.webmanifest", import.meta.url), "utf8"));
  assert.equal(manifest.name, "观象");
  ...
  const runtime = await fs.readFile(new URL("../app/PwaRuntime.tsx", import.meta.url), "utf8");
  const appSource = await fs.readFile(new URL("../app/GuanxiangApp.tsx", import.meta.url), "utf8");
  const styles = await fs.readFile(new URL("../app/globals.css", import.meta.url), "utf8");
  ...
  assert.match(runtime, /window\.visualViewport\?\.height \?\? window\.innerHeight/);
  assert.match(runtime, /style\.setProperty\("--app-height"/);
  assert.match(appSource, /beforeinstallprompt/);
  assert.match(appSource, /appinstalled/);
  assert.match(appSource, /安装观象/);
  assert.match(styles, /\.pwa-install-control/);
  assert.match(styles, /color-scheme:\s*only light/);
  assert.match(runtime, /window\.addEventListener\("pageshow", syncViewportHeight\)/);
  assert.doesNotMatch(serviceWorker, /caches\.|CacheStorage|respondWith/);
});
...
test("first seven pages are single-screen, forward-only and scroll locked", async () => {
  ...
  assert.match(cssSource, /flow-lock-screen[^}]+height: var\(--app-height\) !important[^}]+max-height: var\(--app-height\) !important/);
});
