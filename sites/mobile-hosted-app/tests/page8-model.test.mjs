...
test("page eight is a sticky five-screen scroll story with persistent environmental motion", async () => {
  ...
  assert.match(css, /\.page8-kun-story \{[\s\S]*?height: calc\(\(var\(--page8-scene-count\) \+ 1\) \* var\(--app-height\)\);/);
  assert.match(css, /\.page8-kun-stage \{[\s\S]*?position: sticky;[\s\S]*?height: var\(--app-height\);[\s\S]*?overflow: hidden;/);
...
test("page eight opens from page seven and keeps all later reserved sections hidden", async () => {
  ...
  assert.match(source, /function buildPage8Scaffold\(response: ApiResponse\): Page8Reading \| null/);
  assert.match(source, /const page8Reading = response\.page8_reading \?\? buildPage8Scaffold\(response\)/);
  assert.match(source, /<Page8KunStory reading=\{page8Reading\} task=\{page8Task\}/);
  ...
  assert.match(source, /type Page8TaskPhase = "NOT_REQUESTED" \| "SUBMITTING" \| "RUNNING" \| "SUCCESS" \| "FAILED" \| "RECOVERABLE" \| "TIMEOUT"/);
  assert.match(source, /function Page8TaskPanel/);
  assert.match(source, /详细解卦正在生成；五幕卦象结构已经可以查看，文字完成后会原位出现。/);
  ...
  assert.match(css, /\.page8-reading-open \.flow-shell \{ height: auto; min-height: var\(--app-height\); overflow: visible; \}/);
  ...
  assert.match(css, /\.page9-finale-open \.flow-shell \{ height: var\(--app-height\); min-height: var\(--app-height\); overflow: hidden; \}/);
