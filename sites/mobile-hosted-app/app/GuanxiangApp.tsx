...
function MobileFrozenEntry({ onEnter }: { onEnter: () => void }) {
  ...
  function enter() {
    ...
    const isEmbeddedPreview = new URLSearchParams(window.location.search).get("mobile-preview") === "1";
    if (!isEmbeddedPreview && window.matchMedia("(display-mode: browser)").matches && !document.fullscreenElement) {
      void document.documentElement.requestFullscreen?.({ navigationUI: "hide" }).catch(() => {
        // Fullscreen requires browser support and an accepted user gesture.
      });
    }
    setEntering(true);
    const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
...
function FullscreenRecoveryControl({ flowPage }: { flowPage: number }) {
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    const browserMode = window.matchMedia("(display-mode: browser)");
    const embeddedPreview = new URLSearchParams(window.location.search).get("mobile-preview") === "1";
    const sync = () => {
      setVisible(!embeddedPreview && flowPage > 1 && browserMode.matches && !document.fullscreenElement);
    };
    ...
    browserMode.addEventListener("change", sync);
    return () => {
      document.removeEventListener("fullscreenchange", sync);
      document.removeEventListener("visibilitychange", sync);
      window.removeEventListener("focus", sync);
      window.removeEventListener("pageshow", sync);
      browserMode.removeEventListener("change", sync);
    };
  }, [flowPage]);
  ...
  return <button type="button" className="fullscreen-recovery" onClick={restoreFullscreen} aria-label="恢复全屏显示">
    恢复全屏
  </button>;
}
...
function PwaInstallControl({ flowPage }: { flowPage: number }) {
  ...
  const isEmbeddedPreview = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("mobile-preview") === "1";
  if (flowPage !== 1 || !installPrompt || isEmbeddedPreview) return null;
  return <button type="button" className="pwa-install-control" onClick={install} aria-label="将观象安装到手机桌面">
    安装观象
  </button>;
}
...
export function GuanxiangApp() {
  ...
  return <>
    <PwaInstallControl flowPage={flowPage} />
    <FullscreenRecoveryControl flowPage={flowPage} />
