...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (upstream.status === 401 || upstream.status === 403) {
      if (job.state === "RUNNING") await markDirectReadingPreviewJobLost(requestId, "ENGINE_AUTH_MISMATCH");
      return safeJson({ error: "解卦服务连接配置尚未同步，请稍后重新尝试。", terminal: true, retryable: true }, 503);
    }
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (upstream.status === 401 || upstream.status === 403) {
      await markDirectReadingPreviewJobLost(requestId, "ENGINE_AUTH_MISMATCH");
      return safeJson({ error: "解卦服务连接配置尚未同步，请稍后重新尝试。", terminal: true, retryable: true }, 503);
    }
