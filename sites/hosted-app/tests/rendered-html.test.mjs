...
test("server-renders the Guanxiang product", async () => {
  ...
  assert.match(html, /entry-ink-reveal/);
...
test("method lines retain the replayable writing interaction", async () => {
  ...
  assert.match(appSource, /function MethodRiverFlow/);
  assert.match(appSource, /float downstream = screen\.y - u_time \* mix/);
  assert.match(appSource, /float bank_fade = 1\.0 - smoothstep/);
  assert.match(appSource, /float rapid_belt =/);
  assert.match(appSource, /float belt_breakup =/);
  assert.match(appSource, /float bank_impact =/);
  assert.match(appSource, /float whitewater = clamp/);
  assert.doesNotMatch(appSource, /method-river-current-flow-v2\.png/);
  assert.match(cssSource, /method-river-flow\.is-ready ~ \.method-landscape/);
  assert.match(cssSource, /prefers-reduced-motion:[^}]+reduce[\s\S]+\.method-river-flow \{ display: none; \}/);
});
