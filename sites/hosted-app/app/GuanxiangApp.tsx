...
type TrigramName = typeof TRIGRAMS[number];

function trigramAt(index: number): TrigramName {
  return TRIGRAMS[((index % TRIGRAMS.length) + TRIGRAMS.length) % TRIGRAMS.length];
}
...
function TrigramMark({ trigram = "qian", className = "", decorative = true }: { trigram?: TrigramName; className?: string; decorative?: boolean }) {
  return <img
    className={`trigram-mark ${className}`}
    src={`/trigrams/${trigram}.png`}
    alt={decorative ? "" : "标准八经卦符号"}
...
function ChoiceMenu({ label, value, options, disabled = false, onChange }: {
  ...
  const selectedIndex = Math.max(0, Object.keys(options).indexOf(value));
  return <div className={`choice-field ${disabled ? "is-disabled" : ""}`}>
    ...
    <details className="choice-menu">
      <summary aria-label={`${label}：${selected}`} aria-disabled={disabled} onClick={(event) => { if (disabled) event.preventDefault(); }}>
        <TrigramMark trigram={trigramAt(selectedIndex)} />
        <b>{selected}</b>
      </summary>
      <div role="listbox" aria-label={label}>
        {Object.entries(options).map(([key, text], index) => <button
          key={key}
          type="button"
          role="option"
        ...
        ><TrigramMark trigram={trigramAt(index)} />{text}</button>)}
      </div>
    </details>
  </div>;
}
...
function OptionList({ name, value, options, onChange }: { name: string; value: string; options: Record<string, string>; onChange: (value: string) => void }) {
  return <div className="option-list" role="radiogroup" aria-label={name}>{Object.entries(options).map(([key, label], index) => (
    <label key={key} className={value === key ? "selected" : ""}>
      <input type="radio" name={name} value={key} checked={value === key} onChange={() => onChange(key)} />
      <TrigramMark trigram={trigramAt(index)} />
      <span>{label}</span>
    </label>
  ))}</div>;
}
...
function JournalSection({ records, loading, message, onOpen, onUpdate, onDelete, onExport }: {
  ...
  return <section id="journal" className="journal scroll-section" data-reveal>
    ...
    {loading ? <p className="journal-empty">正在打开观事簿……</p> : records.length ? <div className="journal-list">{records.map((record) => <JournalEntry key={record.id} record={record} onOpen={onOpen} onUpdate={onUpdate} onDelete={onDelete} />)}</div> : <div className="journal-empty"><TrigramMark trigram="gen" /><p>这里还没有记录。完成一次观象后，主动点击“存入观事簿”即可开始。</p></div>}
...
function ResultView({ response, onEdit, onClear, onSave, saving, saved }: { response: ApiResponse; onEdit: () => void; onClear: () => void; onSave: (action: string, reviewOn: string | null) => Promise<void>; saving: boolean; saved: boolean }) {
  ...
  const [action, setAction] = useState(report.next_action);
  ...
  return <section id="result" className="result-shell" aria-labelledby="result-title">
    ...
    <section id="result-signals" className="quick-reading scroll-section" data-reveal>
      <VerticalBrand />
      <header className="section-heading"><p className="eyebrow">先带走最有用的部分</p><h2>一个行动，三个信号</h2><p>不必一次读完所有卦理。先把行动放回现实，再观察下面三个条件是否出现。</p></header>
      ...
      <div className="save-observation">
        <label><span>我准备采取的行动</span><textarea value={action} maxLength={500} onChange={(event) => setAction(event.target.value)} /></label>
        <label><span>回来复盘的日期</span><input type="date" value={reviewOn} onChange={(event) => setReviewOn(event.target.value)} /></label>
        ...
        <small>保存是可选的。保存后，问题、结果与复盘会与当前浏览器中的随机凭据相连；可在观事簿导出或删除。</small>
      </div>
      <nav className="result-jump"><a href="#why-reading">为什么这样判断</a><a href="#deep-reading">深入了解排盘</a><a href="#journal">稍后去观事簿复盘</a></nav>
    </section>
    ...
    <section id="deep-reading" className="evidence-scroll layered-reading scroll-section" data-reveal>
      <VerticalBrand />
      <header className="section-heading"><p className="eyebrow">第三层 · 深入理解</p><h2>卦从何来，变化落在哪里</h2><p>这里保留完整的三数成卦、动爻、体用和旺衰解释，供需要核对依据的人展开阅读。</p></header>
      <details className="reading-disclosure"><summary><span>三数如何成卦</span><small>第一数定上卦 · 第二数定下卦 · 第三数定动爻</small></summary>{cultural && <div className="number-path">{cultural.number_path.map((item, index) => <article key={item.role}><span>{["壹", "贰", "叁"][index]}</span><b>{item.input_number}</b><i aria-hidden="true">→</i><strong>{item.role} · {item.result_name}</strong><small>{item.explanation}</small></article>)}</div>}</details>
      <details className="reading-disclosure"><summary><span>本卦、互卦与变卦</span><small>起始结构 · 中间过程 · 变化方向</small></summary><div className="hexagram-route">{[{ label: "本卦", value: result.base_hexagram }, { label: "互卦", value: result.mutual_hexagram }, { label: "变卦", value: result.changed_hexagram }].map(({ label, value }) => <article key={label}><span>{label}</span><strong>{value.symbol}</strong><h3>{value.name}</h3><small>第 {value.king_wen_number} 卦</small></article>)}</div></details>
      {cultural && <details className="reading-disclosure"><summary><span>本次动爻</span><small>{cultural.moving_line.line_name} · {cultural.moving_line.stage}</small></summary><article className="moving-line-reading"><div><span>本次动爻</span><h3>{cultural.moving_line.line_name}</h3><small>{cultural.moving_line.stage}</small></div><blockquote>{cultural.moving_line.canonical_text}</blockquote><p>这条爻辞记录了本次变化所在的位置。它需要与本卦的整体结构、双方关系和当前承接能力一起理解，不能只凭一句原文决定现实结果。</p></article></details>}
...
function CastingLoader() {
  return <div className="casting" role="status"><TrigramMark trigram="kan" /><p><b>正在观象</b><span>排定本卦 · 察看变化 · 整理方向</span></p></div>;
}
    ...
    <main id="top" className="scroll-canvas">
      <section className="hero scroll-section" data-reveal>
        ...
        <div className="hero-copy">
          ...
          <p className="hero-value">用三分钟，把一件拿不准的事，整理成一个可以验证的下一步。</p>
          <p>观象不替你决定，也不预先写好结果。它把卦象的结构、变化与现实中该观察的条件一层层展开，并让你在后来回来复盘。</p>
          <a className="seal-button" href="#method"><TrigramMark trigram="qian" /><span>遇事不决，可问春风</span></a>
        </div>
      </section>
      ...
      <section id="method" className="method scroll-section" data-reveal>
        ...
        <div className="method-explainer"><p>所谓观象的意思，就是观察身边的现象，由可见之形察其关系，由变化之中辨其趋向。它不是一句含混的预言，而是一条从所问、取数、成卦到现实验证的观察路径。</p>
          ...
          <div className="input-roles"><h3>你写下的内容，不会偷偷改变卦象</h3><p><b>三个数字</b>决定卦象；<b>现实处境</b>帮助安排解释重点；<b>问题原文</b>只用于把事情说清楚和在结果中呈现，不参与排盘计算。</p><a href="/about">查看完整方法与规则版本</a></div>
      ...
      <section id="inquiry" className="inquiry scroll-section" data-reveal>
        ...
        <form onSubmit={submit} noValidate>
          <header className="inquiry-heading"><p className="eyebrow">正问</p><h2>只问一件事，写到可以验证</h2><p>整个过程约三分钟。带星号的内容需要填写。</p></header>

          <section className="inquiry-step"><div className="step-heading"><span>壹</span><div><h3>写清所问</h3><p>尽量写清对象、当前选择和现实范围。</p></div></div>
            <label className="question-label"><span>你真正想问的问题 *</span><textarea aria-label="你真正想问的问题" placeholder="例如：这次合作，我还应该继续投入吗？" value={question} maxLength={160} onChange={(event) => setQuestion(event.target.value)} /><small>{question.trim().length} / 160 · 问题原文只用于理解与呈现，不参与排盘</small></label>
            <div className="question-examples"><span>可以这样问</span><button type="button" onClick={() => setQuestion("这次合作，我还应该继续投入吗？")}>这次合作，我还应该继续投入吗？</button><button type="button" onClick={() => setQuestion("关于现在的工作选择，我下一步最该验证什么？")}>关于现在的工作选择，我下一步最该验证什么？</button></div>
          </section>

          <section className="inquiry-step"><div className="step-heading"><span>贰</span><div><h3>说明现实处境</h3><p>这些信息不会改变卦象，只帮助结果把重点放在你真正需要观察的地方。</p></div></div>
          ...
          <button className="cast-button" disabled={loading}><TrigramMark trigram="li" />{loading ? "正在观象" : "观卦"}</button>
      ...
      <JournalSection records={records} loading={journalLoading} message={journalMessage} onOpen={openObservation} onUpdate={updateObservation} onDelete={deleteObservation} onExport={exportJournal} />
