...
export default function PrivacyPage() {
  return <InfoLayout eyebrow="隐私说明" title="你的问题，应当由你掌握" lead="不保存也可以正常完成一次观象。只有当你主动点击“存入观事簿”时，本站才会保存这次问题、结果和复盘内容。">
    ...
    <section><h2>生成个性化解读时</h2><p>当你点击“观卦”时，问题、处境、已确认事实、未知项与排盘证据会发送给解读模型，用于生成本次结果。模型请求关闭内容存储；站内临时任务最长约 30 分钟，只用于页面查询同一任务。只有当你随后主动点击“存入观事簿”，问题与结果才会进入观事簿。系统会长期保留不含问题原文和模型原文的任务状态与费用统计，用来防止重复生成和控制使用量。</p></section>
