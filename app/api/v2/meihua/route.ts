...
function upstreamUrl(): URL | null {
  ...
  try {
    ...
    url.pathname = `${url.pathname.replace(/\/$/, "")}/api/v2/meihua`;
    url.search = "";
    url.hash = "";
    return url;
  } catch {
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (Number.isFinite(declaredLength) && declaredLength > MAX_REQUEST_BYTES) {
    return safeJson({ error: "请求内容过大。" }, 413);
  }
  ...
  if (!url || !engineKey) {
    return safeJson({ error: "排盘服务尚未连接，请稍后再试。" }, 503);
  }
  ...
  try {
    ...
    if (new TextEncoder().encode(responseText).byteLength > MAX_RESPONSE_BYTES) {
      return safeJson({ error: "排盘服务响应异常。" }, 502);
    }
    const payload = JSON.parse(responseText) as { contract_version?: unknown };
    if (payload.contract_version !== "SITES_MEIHUA_API_CONTRACT_V2") {
      return safeJson({ error: "排盘服务响应异常。" }, 502);
    }
    return safeJson(payload, upstream.ok ? 200 : 502);
  } catch {
    return safeJson({ error: "暂时无法连接排盘服务，请稍后再试。" }, 503);
  }
}
