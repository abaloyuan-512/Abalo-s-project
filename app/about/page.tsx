...
export default function AboutPage() {
  return <InfoLayout eyebrow="方法与边界" title="让每一句判断，都能找到来处" lead="观象是一套以梅花易数确定性排盘为基础、以现实验证为归宿的观察工具。它不替人预言不可改变的结果。">
    <section><h2>三类信息，各有各的作用</h2><dl><div><dt>你写下的问题与现实信息</dt><dd>用于把事情说清楚，并帮助个性化解读落到当前处境；它们不参与排盘计算，未知内容也不会被补写成事实。</dd></div><div><dt>你给出的三个数字</dt><dd>依版本化的三数起卦规则，确定上卦、下卦和动爻，是本次排盘唯一的计算输入。</dd></div><div><dt>程序排出的卦象证据</dt><dd>与现实陈述分开提供给解读模型。模型只能解释这些材料，不能重新排盘，也不能改变本卦、互卦和变卦。</dd></div></dl></section>
    <section><h2>当前采用的规则</h2><p>确定性排盘规则为 <code>MEIHUA_RULE_SPEC_V1</code>，个性化页面接口为 <code>SITES_PERSONALIZED_MEIHUA_CONTRACT_V1</code>。算法规则如需改变，必须升级版本并通过自动化测试；AI 不参与确定卦象。</p></section>
